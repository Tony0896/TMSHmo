                ...
                function(tx, results){
                    ...
                    if(tipo == "checklist"){
                        ...
                        if(item.nombre_cliente == 'Campaña'){
                            ...
                            databaseHandler.db.transaction(
                                function(tx){
                                    tx.executeSql("SELECT *, REPLACE(fechaFin, ' ', 'T') as fechaFin2, REPLACE(fechaInicio, ' ', 'T') as fechaInicio2 FROM IEN_Header WHERE id_cedula = ?",
                                        [id_cedula],
                                        function(tx, results){
                                            var length = results.rows.length;
                                            for(var i = 0; i< length; i++){
                                            ...
                                            databaseHandler.db.transaction(
                                                function(tx){
                                                    tx.executeSql("SELECT * FROM IEN_Details WHERE id_cedula = ?",
                                                        [id_cedula],
                                                        function(tx, results){
                                                            var length = results.rows.length;
                                                            for(var i = 0; i< length; i++){
                        ...
                        databaseHandler.db.transaction(
                            function(tx){
                                tx.executeSql("SELECT * FROM datos_generales_diesel WHERE id_cedula = ?",
                                    [id_cedula],
                                    function(tx, results){
                                        var length = results.rows.length;
                                        for(var i = 0; i< length; i++){
                                        ...
                                        databaseHandler.db.transaction(
                                            function(tx){
                                                tx.executeSql("SELECT * FROM detalle_diesel WHERE id_cedula = ?",
                                                    [id_cedula],
                                                    function(tx, results){
                                                        var length = results.rows.length;
                                                        for(var i = 0; i< length; i++){
